...
/**
 * {@link #getSelling(String, boolean, String)}
 * @author fangzw
 *
 */
public class PingYin {
	...
	private static PingYin characterParser = new PingYin();
	...
	public static PingYin getInstance() {
	...
	private int getChsAscii(String chs) {
		...
		try {
			...
			if (bytes == null || bytes.length > 2 || bytes.length == 0) { //字符格式不对
	 ...
	 * 
	 * @param chs ±ª◊™ªªµƒ◊÷∑˚¥Æ
	 * @param onlyFirstChar  «∑Ò÷ª”√µ⁄“ª∏ˆ◊÷ƒ∏
	 * @param unknowChar ≤ª»œ ∂µƒ◊÷”√ ≤√¥◊÷∑˚±Ì æ
	 * @return ◊™ªªπ˝∫Ûµƒ◊÷∑˚¥Æ
	 */
	public String getSelling(String chs, boolean onlyFirstChar, String unknowChar) {
		if(chs == null) return "";
		String key, value;
		...
		for (int i = 0; i < chs.length(); i++) {
			...
			if (value != null && value.length() > 0 && onlyFirstChar)
	...
	public static void main(String[] args) {
		...
		System.out.println(parser.getSelling("可我忘了", true,""));
	}
}