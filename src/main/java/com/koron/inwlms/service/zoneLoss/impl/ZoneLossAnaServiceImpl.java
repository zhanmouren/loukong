...
public class ZoneLossAnaServiceImpl implements ZoneLossAnaService {
	...
	public PageListVO<List<FZoneLossListVO>> queryFZoneLossList(SessionFactory factory,
		...
		for (int i = 0;i<zoneInfos.size();i++) {
			...
			fZoneLossListVO.setFlow(Double.parseDouble(df.format(flow/10000/timeNum)));
			fZoneLossListVO.setUseFlow(Double.parseDouble(df.format(useFlow/10000/timeNum)));
			fZoneLossListVO.setLossFlow(Double.parseDouble(df.format(lossFlow/10000/timeNum)));
	...
	public PageListVO<List<SZoneLossListVO>> querySZoneLossList(SessionFactory factory, QuerySZoneLossListDTO querySZoneLossListDTO) {
		...
		for (int i = 0; i<zoneInfos.size(); i++) {
			...
			sZoneLossListVO.setFlow(Double.parseDouble(df.format(flow/10000/timeNum)));
			sZoneLossListVO.setUseFlow(Double.parseDouble(df.format(useFlow/10000/timeNum)));
			sZoneLossListVO.setLossFlow(Double.parseDouble(df.format(lossFlow/10000/timeNum)));
	...
	public PageListVO<List<DmaZoneLossListVO>> queryDmaZoneLossList(SessionFactory factory,
		...
		for (int i = 0;i<zoneInfos.size();i++) {
			...
			dmaZoneLossListVO.setFlow(Double.parseDouble(df.format(flow/10000/timeNum)));
			dmaZoneLossListVO.setUseFlow(Double.parseDouble(df.format(useFlow/10000/timeNum)));
			dmaZoneLossListVO.setLossFlow(Double.parseDouble(df.format(lossFlow/10000/timeNum)));
	...
	public PageListVO queryZoneIndicatorList(SessionFactory factory,
		...
		for (int i = 0; i<zoneNos.size();i++) {
			...
			for (String code : codes) {
				...
				if(code.contains("DCPL") ||  
						code.contains("MNF") || code.contains("MNFLF") || code.contains("LCA") || code.contains("LPL")) {
