...
public final class DateTimeUtil {
    ...
    /**
     * 把时间格式化成如：2002 格式的(4位)字符串
     */
    public final static String FMT_yyyyMM_7 = "yyyy";
     ...
     * @return
     */
    public static Integer countMonthOfYear(String year, String startDay, String endDay) {
        Integer result = 0;
        List<String> monthList = getYearBetween(startDay, endDay);
        for (String month : monthList) {
            if (month.substring(0, 4).equals(year)) {
                result++;
            }
        }
        return result;
     ...
     * @return
     */
    public static List<String> getMinuteBetween(String minDate, String maxDate) {
        Date start = dayStartDate(minDate);
        Date end = null;
        try {
            end = nextDay(stringToDate(maxDate, FMT_yyyyMMdd));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        List<String> result = new ArrayList<>();
        while (start.compareTo(end) < 0) {
            result.add(dateToString(start, FMT_yyyyMMddHHmm));
     ...
     * @return
     */
    public static Date dayStartDate(String date) {
        Calendar c = Calendar.getInstance();
        try {
            c.setTime(stringToDate(date, FMT_yyyyMMdd));
        } catch (ParseException e) {
            e.printStackTrace();
        }
    ...
    public static String stringSupMonth(String date, String format) {
    ...
    public static String getLastDayOfMonth(String yearMonth) {
        ...
        // 获取某月最大天数
        int lastDay = month == 2 ? 28 : cal.getActualMaximum(Calendar.DATE);
        // 设置日历中月份的最大天数
     ...
     * @return
     */
    public static Integer getCountDayBetween(String minDate, String maxDate) {
        List<String> temp = getDayBetween(minDate, maxDate);
        return temp.size();
    }
     ...
     * @return
     */
    public static Integer getCountDayBetween(String minDate, String maxDate, String dateType) {
        minDate = minDate.replace("-", "").replace("/", "");
        maxDate = maxDate.replace("-", "").replace("/", "");
        switch (dateType) {
            case "D":
            ...
            case "M":
                ...
                maxDate = getLastDayOfMonth(maxDate.substring(0, 4) + "-" + maxDate.substring(4, 6));
                break;
            case "Y":
    ...
    public static List<String> getDayBetween(String minDate, String maxDate) {
        ...
        String min = getFixDateString(minDate, "yyyy-MM-dd");
        if (min == null || min.isEmpty()) {
            return betweenList;
        }

        String max = getFixDateString(maxDate, "yyyy-MM-dd");
        ...
        try {
            ...
            minCal.setTime(format.parse(min));
            Calendar maxCal = Calendar.getInstance();
            maxCal.setTime(format.parse(max));
            int offsetDay = Math.abs((int) getDaysOfTwoDate(minCal.getTime(), maxCal.getTime()));

            Calendar indexCal = minCal;
            for (int i = 0; i < offsetDay + 1; i++) {
                Date newDate = indexCal.getTime();
    ...
    public static List<String> getDateBetween(String minDate, String maxDate, String dateType) {
        switch (dateType) {
            case "D":
                return getDayBetween(minDate, maxDate);
            case "M":
                return getMonthBetween(minDate, maxDate);
            case "Y":
                return getYearBetween(minDate, maxDate);
            default:
                return null;
        }
    }
    ...
    public static List<String> getMonthBetween(String minDate, String maxDate) {
        ...
        minDate = minDate.replace("-", "").replace("/", "");
        maxDate = maxDate.replace("-", "").replace("/", "");
        minDate = minDate.substring(0, 4) + "-" + minDate.substring(4, 6);
        maxDate = maxDate.substring(0, 4) + "-" + maxDate.substring(4, 6);
        try {
    ...
    public static List<String> getYearBetween(String minDate, String maxDate) {
        List<String> result = new ArrayList<String>();
        int size = Integer.parseInt(maxDate) - Integer.parseInt(minDate);
        for (int i = 0; i <= size; i++) {
            result.add(String.valueOf(Integer.parseInt(minDate) + i));
        }
        return result;
    }
     ...
     * @return
     */
    public static Date supDay(Date dayid) {
     ...
     * @return
     */
    public static Date stepMonth(Date monthid) {
     ...
     * @return
     */
    public static Date supMonth(Date monthid) {
        SimpleDateFormat format = new SimpleDateFormat("yyyyMM");
        Calendar c = Calendar.getInstance();
        c.setTime(monthid);
        c.add(Calendar.MONTH, -1);
     ...
     * @return
     */
    public static Date supYear(Date year) {
     ...
     * @return
     */
    public static Date stepYear(Date year) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Calendar c = Calendar.getInstance();
        c.setTime(year);
    ...
    public static String getFixDateString(String dateString, String format) {
        if (isLegalDate(dateString, format)) {
            return dateString;
        }
        String[] dateArray = dateString.split("-");
        String string = dateArray[0];
        for (int i = 1; i < dateArray.length; i++) {
            if (dateArray[i].length() < 2)
                string += "-0" + dateArray[i];
            else
                string += "-" + dateArray[i];
        }
        if (isLegalDate(string, format)) return string;
     ...
     * @throws ParseException
     */
    public static boolean isLessPreMonthFirstDate(String time, String format) throws ParseException {
        Calendar calendar = Calendar.getInstance();
        DateFormat formatter = new SimpleDateFormat(format);
        ...
        calendar1.set(Calendar.YEAR, Integer.parseInt(split[0]));
        calendar1.set(Calendar.MONTH, Integer.parseInt(split[1]));
        int lastDay = calendar1.getActualMaximum(Calendar.DAY_OF_MONTH);
        Date date1 = formatter.parse(time + "-" + lastDay);
        //获取某月最大天数
        boolean isLess = true;
        if (date1.getTime() - date.getTime() < 0) isLess = false;
        return isLess;
    }
}
